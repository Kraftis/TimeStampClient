...
namespace Disig.RemoteHsm.Web.Infrastructure.Proxy
{
    ...
    public class ProxyMiddleware
    {
        ...
        public Task Invoke(HttpContext context, ProxyService proxyService)
        {
            if (context == null)
            {
            ...
            if(!proxyService.FilterRequest(context.Request))
            {
                context.Response.StatusCode = 500;
                byte[] response = Encoding.UTF8.GetBytes("Request rejected proxy.");
                context.Response.Body.Write(response, 0, response.Length);

                return Task.CompletedTask;
            }

            Uri uri = new Uri(UriHelper.BuildAbsolute(_options.Scheme, _options.Host, _options.PathBase, context.Request.Path, context.Request.QueryString.Add(_options.AppendQuery)));
