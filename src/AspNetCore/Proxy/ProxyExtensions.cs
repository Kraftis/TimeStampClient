...
namespace Disig.RemoteHsm.Web.Infrastructure.Proxy
{
    public static class ProxyExtensions
    {
        ...
        public static async Task ProxyRequest(this HttpContext context, Uri destinationUri)
        {
            ...
            {
                ProxyService proxyService = context.RequestServices.GetRequiredService<ProxyService>();

                using (System.Net.Http.HttpRequestMessage requestMessage = context.CreateProxyHttpRequest(destinationUri))
                {
                    Func<HttpRequest, System.Net.Http.HttpRequestMessage, Task> prepareRequestHandler = proxyService.Options.PrepareRequest;
                    if (prepareRequestHandler != null)
                    {
                        await prepareRequestHandler(context.Request, requestMessage);
                    }

                    using (System.Net.Http.HttpResponseMessage responseMessage = await context.SendProxyHttpRequest(requestMessage))
                    {
